...
/*shortcut: only use during development, will return that same set of results that the first query( all of the rows in that table.).  */
SELECT * FROM Song;
...
/*SELECT query that lists all the songs in the Song table and include the Artist name*/
SELECT Song.*, Artist.ArtistName
FROM Song
LEFT JOIN Artist ON Song.ArtistId = Artist.Id;

/* SELECT query that lists all the Artists that have a Pop Album*/
...
/*SELECT query that lists all the Artists that have a Jazz or Rock Album*/
SELECT Artist.ArtistName
FROM Artist
JOIN Album ON Album.ArtistId = Artist.Id
JOIN Genre ON Album.GenreId = Genre.Id
Where Genre.Id = 2 or  Genre.Id =4;

/*SELECT statement that lists the Albums with no songs*/
--Select * from Song s
